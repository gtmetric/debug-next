#!/usr/bin/env node
/**
 * CLI wrapper for `next dev` (or any long-running build command). Tees
 * the child process's stdout/stderr verbatim to `<appName>.log` so the
 * AI copilot can read exactly what the developer sees in the terminal.
 *
 *   debug-next-dev -- next dev --turbopack
 *
 * The log file is truncated once at startup, so each `bun run dev`
...
const USAGE = [
    ...
    '',
    'Wraps a Next.js dev/build command, tees its output to the terminal,',
