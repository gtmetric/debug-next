...
export type TAttachFileWriterOptions = {
     ...
     * host has already initialised LogBase (e.g. an Express app already
 ...
 * Idempotent: silently no-ops if `LogBase` is already initialised or
...
const formatRequestError = (
    ...
    const header = `[${isoTs()}] [${appName}] logError ${route} — ${
        reqLine || '(no request)'
    }`
